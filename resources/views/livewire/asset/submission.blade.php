...
<div class="max-w-5xl p-2 mx-auto my-4 space-y-3 md:p-0" x-data="{ selected:1 }">
    <div class="flex items-center justify-center gap-2 mb-6">
        <a href="{{ route('asset.submission') }}" class="px-4 py-2 text-primary-foreground bg-primary rounded-full" wire:navigate>
            {{ __('Senarai Permohonan') }}
        </a>
        <a href="{{ route('asset.listing') }}" class="px-4 py-2 transition rounded-full hover:bg-accent"
            wire:navigate>
            {{ __('Senarai Aset') }}
        </a>
    </div>
    <button class="w-full px-8 py-3 text-xl font-medium tracking-wide bg-muted rounded-md"
        x-on:click.prevent="selected !== 1 ? selected = 1 : selected = null"
        x-bind:class="selected == 1 ? 'transition bg-[radial-gradient(ellipse_at_bottom,_var(--tw-gradient-stops))] from-accent via-muted to-muted ring-2 ring-ring ring-offset-2 ' : ''">
                        ...
                        <td class="p-3 rounded-r-lg">
                            ...
                            <x-modal name="approve-application-{{ $application->id }}" maxWidth="md" focusable>
                                <form wire:submit.prevent="approve({{ $application->id }})" class="p-6 my-4 space-y-2"
                                    ...
                                    @csrf
                                    <div class="relative text-foreground" x-data="select({{ $assets }})"
                                        @click.away="close()">
                                        <p class="block mb-1 font-medium text-foreground">Pilih Aset *</p>
                                        <div class="flex items-center gap-2">
                                            ...
                                            <div class="relative inline-block w-full rounded-md shadow-sm"
                                                ...
                                                <button type="button" autofocus
                                                    class="relative z-0 w-full py-2 pl-3 pr-10 text-left transition duration-150 ease-in-out bg-background border-2 border-border rounded-md cursor-default focus:border-ring sm:leading-5">
                                        ...
                                        <div x-show="state"
                                            class="absolute w-full p-2 mt-1 bg-background rounded-md shadow-lg z-100">
                                            <input type="text"
                                                class="w-full px-2 py-1 mb-1 border border-border rounded-md"
                                            ...
                                            <ul
                                                ...
                                                <template x-for="(value, key) in getlist()" :key="key">
                                                    <li @click="select(value, key)"
                                                        :class="{'bg-muted': isselected(key)}"
                                                        class="relative py-1 pl-3 mb-1 text-foreground rounded-md cursor-pointer select-none pr-9 hover:bg-muted">
    ...
    <button class="w-full px-8 py-3 text-xl font-medium tracking-wide bg-muted rounded-md"
        ...
        x-bind:class="selected == 2 ? 'transition bg-[radial-gradient(ellipse_at_bottom,_var(--tw-gradient-stops))] from-accent via-muted to-muted ring-2 ring-ring ring-offset-2 ' : ''">
    ...
    <div class="relative overflow-hidden transition-all duration-700 max-h-0" style="" x-ref="container2"
        ...
        <div class="px-2 pb-16 overflow-x-auto rounded-lg">
            <table class="w-full mb-4 text-xs text-left text-foreground border-separate table-auto border-spacing-y-2">
                <thead class="text-xs font-medium uppercase bg-muted border-b-2">
                    <tr class="tracking-wide">
                        <th class="px-3 py-4 rounded-l-lg">
                            Tarikh Terima
                        </th>
