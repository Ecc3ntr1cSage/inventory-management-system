...
<div class="max-w-md p-2 mx-auto mt-4 md:p-0">
    <div class="flex items-center justify-center gap-2 mb-6">
        <a href="{{ route('inventory.entry') }}" class="px-4 py-2 text-white bg-gray-800 rounded-full" wire:navigate>
            {{ __('Kemasukan/Keluaran') }}
        </a>
        <a href="{{ route('inventory.listing') }}" class="px-4 py-2 transition rounded-full hover:bg-gray-300"
            wire:navigate>
    ...
    <form wire:submit.prevent="entry" class="my-6 space-y-4" enctype="multipart/form-data">
        @csrf
        <div class="relative text-black" x-data="select({{ $stocks }})" @click.away="close()">
            <p class="block mb-1 font-medium text-gray-700">Perihal Stok *</p>
            <div class="flex items-center gap-2">
                ...
                <div class="relative inline-block w-full rounded-md shadow-sm"
                    ...
                    <button type="button" autofocus
                        class="relative z-0 w-full py-2 pl-3 pr-10 text-left transition duration-150 ease-in-out bg-white border border-gray-300 rounded-md cursor-default focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:leading-5">
